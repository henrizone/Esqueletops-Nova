...
export async function listStickerPacks(userId:number):Promise<StickerPackRecord[]>{return(await db.query<any>("SELECT id,user_id,pack_name,title,format,is_default FROM sticker_packs WHERE user_id=$1 ORDER BY is_default DESC,created_at",[userId])).rows.map(packMap);}
export async function getDefaultStickerPack(userId:number):Promise<StickerPackRecord|null>{const x=(await db.query<any>("SELECT id,user_id,pack_name,title,format,is_default FROM sticker_packs WHERE user_id=$1 AND is_default=TRUE LIMIT 1",[userId])).rows[0];return x?packMap(x):null;}
export async function createStickerPack(input:{userId:number;packName:string;title:string;format:StickerFormat;makeDefault?:boolean}):Promise<StickerPackRecord>{
  const c=await db.connect();try{await c.query("BEGIN");if(input.makeDefault!==false)await c.query("UPDATE sticker_packs SET is_default=FALSE WHERE user_id=$1",[input.userId]);
...
export async function setDefaultStickerPack(userId:number,packId:number):Promise<boolean>{const c=await db.connect();try{await c.query("BEGIN");const exists=(await c.query("SELECT 1 FROM sticker_packs WHERE id=$1 AND user_id=$2",[packId,userId])).rowCount;if(!exists){await c.query("ROLLBACK");return false;}await c.query("UPDATE sticker_packs SET is_default=FALSE WHERE user_id=$1",[userId]);await c.query("UPDATE sticker_packs SET is_default=TRUE,updated_at=NOW() WHERE id=$1",[packId]);await c.query("COMMIT");return true;}catch(e){await c.query("ROLLBACK");throw e;}finally{c.release();}}
export async function updateStickerPackTitle(userId:number,packId:number,title:string):Promise<boolean>{return((await db.query("UPDATE sticker_packs SET title=$1,updated_at=NOW() WHERE id=$2 AND user_id=$3",[title,packId,userId])).rowCount??0)>0;}
export async function deleteStickerPackRecord(userId:number,packId:number):Promise<StickerPackRecord|null>{const x=(await db.query<any>("DELETE FROM sticker_packs WHERE id=$1 AND user_id=$2 RETURNING id,user_id,pack_name,title,format,is_default",[packId,userId])).rows[0];if(!x)return null;if(x.is_default)await db.query("UPDATE sticker_packs SET is_default=TRUE,updated_at=NOW() WHERE id=(SELECT id FROM sticker_packs WHERE user_id=$1 ORDER BY updated_at DESC,created_at DESC LIMIT 1)",[userId]);return packMap(x);}
